...
app.use(express.static(path.join(__dirname, 'public'), {
  ...
  setHeaders(res, filePath) {
    ...
    if (/\.(html|css|js|apk|ipa)$/i.test(filePath)) {
...
const translationMap = {
  'sms-activate-shutdown-alternatives': {
    ...
    description: 'SMS-Activate resmi ditutup permanen setelah 10 tahun beroperasi. Teknologi dialihkan ke VTech OTP Server. Simak kronologi lengkap, peringatan penipuan, dan 5 alternatif nomor virtual terpercaya.'
  },
  ...
  'best-countries-for-virtual-numbers-2026': {
    ...
    description: 'Perbandingan nomor virtual Indonesia, AS, Inggris, Rusia, dan India — tingkat keberhasilan verifikasi, harga nomor virtual, dan rekomendasi platform.'
  },
  ...
  'free-vs-paid-virtual-numbers-comparison': {
    ...
    description: 'Perbandingan lengkap nomor virtual gratis vs berbayar. Risiko kebocoran data, pemblokiran akun, dan keunggulan nomor virtual privat VTechOTP.'
  },
  'instagram-disabled-recovery-guide': {
    title: 'Akun Instagram Dinonaktifkan? Cara Pemulihan & Buat Akun Baru dengan Nomor Virtual (2026)',
  ...
  'multiple-whatsapp-accounts-guide': {
    title: 'Cara Menjalankan Banyak Akun WhatsApp dalam 1 HP dengan Nomor Virtual (2026)',
    description: 'Panduan membuat dan mengaktifkan multiple akun WhatsApp dalam satu smartphone menggunakan nomor virtual OTP VTechOTP.'
  },
  ...
  'telegram-channel-virtual-number-guide': {
    ...
    description: 'Panduan mendaftar akun Telegram dan membuat channel publik/privat secara aman menggunakan nomor virtual Telegram VTechOTP.'
  },
  'temporary-vs-permanent-virtual-numbers': {
    title: 'Nomor Virtual Sementara vs Permanen — Mana Yang Anda Butuhkan? (2026)',
    description: 'Memahami perbedaan nomor virtual sekali pakai (temporary) dengan sewa nomor jangka panjang untuk verifikasi berkala.'
  },
...
// Dedicated APK & IPA download endpoint with robust streaming & MIME headers
app.get([
  '/downloads/VTechOTP2.apk', '/downloads/VTechOTP-v2.13.apk', '/downloads/2.13.apk',
  '/downloads/VTechOTP-v2.13.ipa', '/downloads/2.13.ipa',
  '/downloads/:file', '/download/:file',
  '/VTechOTP2.apk', '/VTechOTP-v2.13.apk', '/2.13.apk', '/download-apk',
  '/VTechOTP-v2.13.ipa', '/2.13.ipa', '/download-ipa'
], (req, res) => {
  const fs = require('fs');
  const reqUrl = req.path.toLowerCase();
  let requestedFile = req.params.file || path.basename(req.path);
  let isIpa = reqUrl.endsWith('.ipa') || reqUrl.includes('ipa');
  
  let targetFileName = isIpa ? 'VTechOTP-v2.13.ipa' : 'VTechOTP2.apk';
  let filePath = path.join(__dirname, 'public', 'downloads', requestedFile);
  
  if (!fs.existsSync(filePath) || fs.statSync(filePath).isDirectory()) {
    filePath = path.join(__dirname, 'public', 'downloads', targetFileName);
  }

  if (fs.existsSync(filePath)) {
    const stat = fs.statSync(filePath);
    const contentType = isIpa
      ? 'application/octet-stream'
      : 'application/vnd.android.package-archive';
    const downloadName = isIpa ? 'VTechOTP-v2.13.ipa' : 'VTechOTP-v2.13.apk';
    ...
    res.status(404).type('text/plain').send('File unduhan tidak ditemukan');
  }
});

// Page Routes
app.get(['/docs', '/docs.html', '/api-docs', '/api-documentation', '/developer', '/developers'], (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'docs.html'));
});

app.get(['/playstore', '/play-store', '/playstore.html', '/android'], (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'playstore.html'));
});

app.get(['/appstore', '/app-store', '/appstore.html', '/ios', '/iphone'], (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'appstore.html'));
});
...
app.get('/blog/:slug', (req, res, next) => {
  if (/\.(css|js|png|jpg|jpeg|gif|svg|ico|json|woff2?|ttf|eot|apk|ipa)$/i.test(req.params.slug)) {
    return next();
  }
...
// Static Asset 404 Guard — prevent HTML fallback for missing CSS/JS/images/APKs/IPAs
app.use((req, res, next) => {
  if (/\.(css|js|png|jpg|jpeg|gif|svg|ico|json|woff2?|ttf|eot|apk|ipa)$/i.test(req.path)) {
